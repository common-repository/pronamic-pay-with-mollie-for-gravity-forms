...
class PaymentAddOn extends GFPaymentAddOn {
	...
	public function __construct() {
		 ...
		 */
		$this->_title = __( 'Pronamic Pay Add-On', 'pronamic-pay-with-mollie-for-gravity-forms' );
		 ...
		 */
		$this->_short_title = __( 'Pay', 'pronamic-pay-with-mollie-for-gravity-forms' );
	...
	public function pronamic_maybe_save_feed() {
		...
		if ( '' === trim( $post_title ) ) {
			...
			$post_title = __( 'Payment Feed', 'pronamic-pay-with-mollie-for-gravity-forms' );
			...
			if ( $count > 1 ) {
				$post_title = sprintf(
					...
					__( 'Payment Feed #%d', 'pronamic-pay-with-mollie-for-gravity-forms' ),
					$count
				);
			}
		}
	...
	public function feed_list_title() {
		...
		if ( GravityForms::version_compare( '2.5-rc', '>=' ) ) {
			return __( 'Pay', 'pronamic-pay-with-mollie-for-gravity-forms' );
		}

		$title = sprintf(
			'<i class="dashicons dashicons-money fa-"></i> %s',
			esc_html__( 'Pay Feeds', 'pronamic-pay-with-mollie-for-gravity-forms' )
		);
		...
		$title .= sprintf(
			...
			esc_url( $url ),
			esc_html__( 'Add New', 'pronamic-pay-with-mollie-for-gravity-forms' )
		);
	...
	public function feed_list_columns() {
		return [
			'name'                    => esc_html__( 'Name', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'transaction_description' => esc_html__( 'Transaction Description', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'configuration'           => esc_html__( 'Configuration', 'pronamic-pay-with-mollie-for-gravity-forms' ),
		];
	}
	...
	public function get_column_value_name( $feed ) {
		...
		if ( empty( $title ) ) {
			$title = __( 'Default pay feed', 'pronamic-pay-with-mollie-for-gravity-forms' );
		}
	...
	public function feed_list_no_item_message() {
		...
		$label = \__( 'This form doesn’t have any pay feeds. Let’s go %1$screate one%2$s.', 'pronamic-pay-with-mollie-for-gravity-forms' );
	...
	public function supported_notification_events( $form ) {
		...
		$events = [
			'complete_payment'          => esc_html__( 'Payment Completed', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'fail_payment'              => esc_html__( 'Payment Failed', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'add_pending_payment'       => esc_html__( 'Payment Pending', 'pronamic-pay-with-mollie-for-gravity-forms' ),

			// Subscription events.
			'create_subscription'       => esc_html__( 'Subscription Created', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'cancel_subscription'       => esc_html__( 'Subscription Canceled', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'expire_subscription'       => esc_html__( 'Subscription Expired', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'renew_subscription'        => esc_html__( 'Subscription Renewal Notice', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'add_subscription_payment'  => esc_html__( 'Subscription Payment Added', 'pronamic-pay-with-mollie-for-gravity-forms' ),
			'fail_subscription_payment' => esc_html__( 'Subscription Payment Failed', 'pronamic-pay-with-mollie-for-gravity-forms' ),
		];
