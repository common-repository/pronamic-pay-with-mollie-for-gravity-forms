...
class IssuersField extends GF_Field_Select {
		...
		if ( is_admin() ) {
			...
			if ( empty( $feeds ) ) {
				$link = sprintf(
					...
					__( 'New Payment Feed', 'pronamic-pay-with-mollie-for-gravity-forms' )
				);
			...
			if ( ! empty( $feeds ) && empty( $this->choices ) ) {
				...
				$error = sprintf(
					...
					__( 'This field is not supported by your payment gateway.', 'pronamic-pay-with-mollie-for-gravity-forms' ),
					sprintf(
						/* translators: %s: new feed URL */
						__( 'Please remove it from this form or <a href="%s" target="_blank">add a supported payment gateway</a>.', 'pronamic-pay-with-mollie-for-gravity-forms' ),
						esc_url( $new_feed_url )
					)
				);
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Issuer', 'pronamic-pay-with-mollie-for-gravity-forms' );
	}
	...
	public function get_form_editor_button() {
		return [
			...
			'text'  => __( 'Issuer', 'pronamic-pay-with-mollie-for-gravity-forms' ),
		];
	}
	...
	public static function editor_js_set_default_values() {
		$label = __( 'Choose a bank for iDEAL payment', 'pronamic-pay-with-mollie-for-gravity-forms' );
